...
## Cold Start Prompt
Priority: **RUN THE MIGRATIONS** — `mtg-dna/supabase/migrations/002_legends.sql`, `003_legend_identity.sql` (`type_line`/`oracle_text`/`mana_cost`), AND `004_legend_color_identity.sql` (`color_identity text[]`, used to auto-seed the brew swipe queue) must all be run manually in the Supabase SQL editor before the Vault form, Brew save, LegendBox/AddLegendSheet flow, identity-backfill writes, the brew auto-seed, or flick-is-a-write deck_cards persistence will work in prod (all legends/decks/deck_cards writes are unverified live). After that: end-to-end test of Home → LegendBox grid (100-card gate, identity backfill, brightness-lifted gated art) → LegendIdentity → tap a deck row (opens that deck's live review directly with correct resumed tally/counts, × remove writes -1, back goes to swipe to continue brewing with a deduped queue) → restyled "brew" button (direct-to-swipe auto-seeded session, deck-row-on-session-start, each flick writes deck_cards immediately, undo reverts the write, exiting via back is pure navigation) → AddLegendSheet "add legend" flow (now portal-rendered) → nav tab root-reset on a live deck/device. Decide the pile's fate (carousel gesture model made it unreachable — see Known Issues), wire mode-specific behavior in Brew (`brewMode` is stored but only the "legend" session path has real behavior).
